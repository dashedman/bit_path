...
class PredictsPusher:
    def __init__(self):
        self.best_rbs = 0
        self.global_pq_balance = 0
        self.global_abp_balance = 0
        self.tries = set()

        self.propagate_queue: deque[RouteItem] = deque()
        self.rollback_queue: deque[RollbackItem] = deque()
        self.branching_options: SortedList[TreeBitAtom] = SortedList(key=TreeBitAtom.determinancy)
        ...
        for p_bit, e_bit in zip(predicted_h_bits, h_end_bits):
            if True:
                ...
                if isinstance(e_bit.value, bool):
                    self.propagate_queue.append(RouteItem(bit=p_bit, bit_value=e_bit.value))
